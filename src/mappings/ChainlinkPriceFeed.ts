...
export function handleEthUsdAggregatorSet(event: EthUsdAggregatorSet): void {
  ...
  trackCurrencyPrice(eth, event.block.timestamp, BigDecimal.fromString('0'));
  ...
  trackCurrencyPrice(aud, event.block.timestamp, BigDecimal.fromString('0'));

  let btc = ensureCurrency('BTC');
  trackCurrencyPrice(btc, event.block.timestamp, BigDecimal.fromString('0'));

  let chf = ensureCurrency('CHF');
  trackCurrencyPrice(chf, event.block.timestamp, BigDecimal.fromString('0'));

  let eur = ensureCurrency('EUR');
  trackCurrencyPrice(eur, event.block.timestamp, BigDecimal.fromString('0'));

  let gbp = ensureCurrency('GBP');
  trackCurrencyPrice(gbp, event.block.timestamp, BigDecimal.fromString('0'));

  let jpy = ensureCurrency('JPY');
  trackCurrencyPrice(jpy, event.block.timestamp, BigDecimal.fromString('0'));
...
export function handlePrimitiveAdded(event: PrimitiveAdded): void {
  ...
  trackAssetPrice(primitive, event.block.timestamp, BigDecimal.fromString('0'));
}
